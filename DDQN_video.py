...
class SnakeGameAI(gym.Env):
    ...
    def render(self, generation=0):
        ...
        # Function to check if a cell is adjacent
        def is_adjacent(cell1, cell2):
            return (abs(cell1.x - cell2.x) == 1 and cell1.y == cell2.y) or \
                (abs(cell1.y - cell2.y) == 1 and cell1.x == cell2.x)

        # Draw snake segments
        for i, cell in enumerate(self.snake):
            color = (0, 255, 0) if cell.is_head else (0, 150, 0)
            x = self.grid_x_offset + cell.x * self.cell_size
            y = self.grid_y_offset + cell.y * self.cell_size

            # Draw the snake body part
            pygame.draw.rect(self.screen, color, (x, y, self.cell_size, self.cell_size))

            # Determine which sides need a border
            top, bottom, left, right = True, True, True, True

            if i > 0 and is_adjacent(cell, self.snake[i - 1]):  # Check with previous segment
                if self.snake[i - 1].x == cell.x and self.snake[i - 1].y == cell.y - 1:
                    top = False
                if self.snake[i - 1].x == cell.x and self.snake[i - 1].y == cell.y + 1:
                    bottom = False
                if self.snake[i - 1].x == cell.x - 1 and self.snake[i - 1].y == cell.y:
                    left = False
                if self.snake[i - 1].x == cell.x + 1 and self.snake[i - 1].y == cell.y:
                    right = False

            if i < len(self.snake) - 1 and is_adjacent(cell, self.snake[i + 1]):  # Check with next segment
                if self.snake[i + 1].x == cell.x and self.snake[i + 1].y == cell.y - 1:
                    top = False
                if self.snake[i + 1].x == cell.x and self.snake[i + 1].y == cell.y + 1:
                    bottom = False
            ...
            if top:
                pygame.draw.line(self.screen, border_color, (x, y), (x + self.cell_size, y), border_thickness)
            if bottom:
                pygame.draw.line(self.screen, border_color, (x, y + self.cell_size),
                                 (x + self.cell_size, y + self.cell_size), border_thickness)
            if left:
                pygame.draw.line(self.screen, border_color, (x, y), (x, y + self.cell_size), border_thickness)
            if right:
                pygame.draw.line(self.screen, border_color, (x + self.cell_size, y),
                                 (x + self.cell_size, y + self.cell_size), border_thickness)
...
class AgentDDQN:
    ...
    def select_action(self, state, env):
        # Get the current direction of the snake from the environment
        current_direction = env.direction
        direction_map = [(0, -1), (0, 1), (-1, 0), (1, 0)]  # Up, Down, Left, Right
        opposite_direction = (-current_direction[0], -current_direction[1])

        # All possible actions: 0 = Up, 1 = Down, 2 = Left, 3 = Right
        possible_actions = [i for i, dir in enumerate(direction_map) if dir != opposite_direction]

        state_tensor = torch.FloatTensor(state).unsqueeze(0)

        if random.random() > self.epsilon:
            # If not exploring, choose the best action from the allowed actions
            with torch.no_grad():
                q_values = self.current_model(state_tensor)
                # Extract valid actions and pick the one with the highest Q-value
                q_values_for_possible_actions = q_values[0, possible_actions]
                action = possible_actions[torch.argmax(q_values_for_possible_actions).item()]
        else:
            # Randomly explore from valid actions
...
def play_with_model(agent, env, generations=1, epsilon_override=None):
    """Play with the current model and allow exploration using epsilon."""
    if epsilon_override is not None:
        agent.epsilon = epsilon_override  # Override epsilon if provided
    ...
    while not done:
        ...
        # Use epsilon-greedy action selection during gameplay
        action = agent.select_action(state, env)  # This uses the epsilon-greedy policy
        next_state, reward, done, _ = env.step(action)
        state = next_state
        env.render(generation=generations)

        clock.tick(25)  # Control the frame rate, limiting to 10 frames per second
